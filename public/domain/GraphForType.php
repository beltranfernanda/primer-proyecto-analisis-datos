...
class GraphForType {
    ...
    public function showGraphsForType(){
        ...
        for ($i = 0; $i < $array_num; $i++){
            ...
            if($i == 0){
                echo '<button target="_blank" class="nav-link active" id="pillstab'.$i.'" data-bs-toggle="pill" data-bs-target="#tab'.$i.'" type="button" role="tab" aria-controls="tab'.$i.'" aria-selected="true">'.$this->graphsForType[$i]->getTab().'</button>';
            }else{
                echo '<button target="_blank" class="nav-link" id="pills-tab'.$i.'" data-bs-toggle="pill" data-bs-target="#tab'.$i.'" type="button" role="tab" aria-controls="tab'.$i.'" aria-selected="false">'.$this->graphsForType[$i]->getTab().'</button>';
            }
            echo '</li>';
        }
        echo '</ul>';
    ...
    private function showGraphs($graphs){
        ...
        for ($i = 0; $i < $array_num; $i++){
            ...
            $showGraph->setGrid($graphs[$i]->getColumns());
